...
# I think the only modifications I (Alex) made were to make things clearer and add to the _main.py_ test, to make it more full-fledged
# This program is free software; you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# SR Research part last updated on 5/13/2021

import os, time, platform
import array, string
import pylink #this is the Eyelink eyetracker library
import numpy
import psychopy
...
class EyeLinkCoreGraphicsPsychoPy(pylink.EyeLinkCustomDisplay):
    def __init__(self, tracker, win):
        ...
        if not DISABLE_AUDIO:
            try:
                self._target_beep = Sound( os.path.join('eyetrackingCode','type.wav'), stereo=True)
                self._error_beep = Sound(  os.path.join('eyetrackingCode','error.wav'), stereo=True)
                self._done_beep = Sound(   os.path.join('eyetrackingCode','qbeep.wav'), stereo=True)
            except Exception as e:
                print ('Failed to load audio: '+ str(e), ' Therefore disabling audio')
...
def main():
    """ A short script showing how to use this library, modified by Alex
    ...
    if dummy_mode:
        print("Running in dummy_mode, so will not actually try to connect to eyetracker")
        el_tracker = pylink.EyeLink(None)
    else:  # Connect to the tracker
        try:
            el_tracker = pylink.EyeLink("100.1.1.1")
        except RuntimeError as error:
            print('ERROR couldnt connect to the eyetracker:', error)
            core.quit()
            sys.exit()
    ...
    try:
        ...
        print('ERROR when starting EDF file on eyetracker PC:', err)
    ...
    msg_stim = visual.TextStim(win, msg,
                            color=genv.getForegroundColor(), wrapWidth=scn_w/2)
    msg_stim.draw()
    win.flip()
    print(msg)
    ...
    eyetracking_results_folder = " "
    local_edf = os.path.join(eyetracking_results_folder, edf_file)
    try:
        el_tracker.receiveDataFile(edf_file, local_edf)
        print("EDF file ",local_edf," transfer request executed.")
        print("File received = ",os.path.isfile(local_edf))
    except RuntimeError as error:
        print('When trying to get EDF file from eyetracker computer, ERROR:', error)

    # Disconnect from the eyetracker
    el_tracker.close()
