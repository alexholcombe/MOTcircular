...
In $2*pi$ deg of visual angle at 1 deg eccentricity, $2*n$ relevant neurons are present.
At 2 deg eccentricity, $2*pi$ + $2*pi$ neurons are present.

At 1 deg eccentricity, $2*n/(2*pi)$  are present per deg of visual angle
At $e$ deg eccentricity, $2*n/(2*pi*e)$ are present per deg of visual angle

$N(e)$ - neurons at each eccentricity per degree of visual angle

$$N(e) = 2*n/(2*pi*e) = n/(pi*e)$$
...
$$Minv = c/(n/(pi*e)) = (c*pi*e)/n = c*pi*e/n$$

So indeed, as long as $Minv$ is linear, it works because $c$ and $n$ are unknown.
